...
export default function TTGroupSetupPage() {
  ...
  const addPlayerToGroup = async (groupName: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/tt-groups/add-player", {
        method: "POST",
  ...
  const removePlayerFromGroup = async (groupName: string, playerName: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/tt-groups/remove-player", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ groupName, playerName }),
      });
  ...
  const initializeAllGroups = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/tt-groups/initialize", {
        method: "POST",
      });
    ...
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-2">🏓 Table Tennis Group Setup</h1>
