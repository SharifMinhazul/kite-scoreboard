      ...
      <div className={`max-w-7xl mx-auto broadcast-title ${isSlideshow ? 'mb-4' : 'mb-8'}`}>
        <div className={`text-center ${isSlideshow ? 'mb-3' : 'mb-6'}`}>
          <div className={`inline-block ${isSlideshow ? 'mb-2' : 'mb-4'}`}>
            <div className={`font-display tracking-[0.3em] text-accent ${isSlideshow ? 'text-[8px] mb-1' : 'text-xs mb-2'}`}>GROUP STAGE • ROUND ROBIN</div>
...
function GroupStandingsCard({ group, isSlideshow = false }: { group: ITTGroup; isSlideshow?: boolean }) {
  ...
  return (
    <Card className="border-2 border-accent/30 bg-card/50 backdrop-blur-sm stripe-pattern relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 h-1 bg-accent"></div>
      {/* Corner decoration */}
      {!isSlideshow && <div className="absolute bottom-0 right-0 w-12 h-12 border-t-2 border-l-2 border-accent/20"></div>}
